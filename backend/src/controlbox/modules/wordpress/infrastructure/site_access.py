"""Build WordPress site access / credential views for the panel."""
...
def build_site_access_info(
    ...
    *,
...
) -> WordPressSiteAccessInfo:
    creds: dict[str, Any] = {}
    raw = site.settings.get("provision_credentials")
    if isinstance(raw, dict):
        creds = raw

    login_url = str(creds.get("login_url") or f"{site.url.rstrip('/')}/wp-admin")
    ...
    if not db_password and decrypt_db_password and settings and site.settings.get("db_password_enc"):
        try:
            db_password = WordPressProvisioner(settings).decrypt_db_password(
                str(site.settings["db_password_enc"])
            )
        except Exception:
            db_password = None

    return WordPressSiteAccessInfo(
