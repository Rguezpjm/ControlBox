...
class JoomlaSiteModel(Base, TimestampMixin):
    __tablename__ = "joomla_sites"
    __table_args__ = (
        Index("ix_joomla_sites_tenant_id", "tenant_id"),
        Index("ix_joomla_sites_domain", "domain"),
        Index("ix_joomla_sites_status", "status"),
    )
    ...
    joomla_version: Mapped[str] = mapped_column(String(32), nullable=False, default="5.1.1")
    ...
    parent_site_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("joomla_sites.id", ondelete="SET NULL"), nullable=True
    )
...
class JoomlaBackupModel(Base, TimestampMixin):
    __tablename__ = "joomla_backups"
    __table_args__ = (Index("ix_joomla_backups_site_id", "site_id"),)

    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    site_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("joomla_sites.id", ondelete="CASCADE"), nullable=False
    )
    tenant_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False
    )
