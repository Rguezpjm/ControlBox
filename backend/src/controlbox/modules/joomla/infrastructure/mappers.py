...
def site_to_entity(model: JoomlaSiteModel) -> JoomlaSite:
    return JoomlaSite(
        id=model.id,
        ...
        name=model.name,
        domain=model.domain,
        status=JoomlaStatus(model.status),
        php_version=model.php_version,
        joomla_version=model.joomla_version,
        url=model.url,
        ...
        ssl_status=JoomlaSslStatus(model.ssl_status),
...
def site_to_model(entity: JoomlaSite) -> JoomlaSiteModel:
    return JoomlaSiteModel(
        id=entity.id,
        ...
        joomla_version=entity.joomla_version,
        url=entity.url,
...
def backup_to_entity(model: JoomlaBackupModel) -> JoomlaBackup:
    return JoomlaBackup(
        id=model.id,
        site_id=model.site_id,
        tenant_id=model.tenant_id,
        name=model.name,
        status=JoomlaBackupStatus(model.status),
...
def backup_to_model(entity: JoomlaBackup) -> JoomlaBackupModel:
    return JoomlaBackupModel(
        id=entity.id,
